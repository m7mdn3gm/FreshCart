    ...
    <div className="boxs-product flex flex-wrap shadow">
      <div className="box-product relative w-full">
        ...
        <div className="fav flex flex-col bg-[#fff] shadow rounded">
          <button onClick={()=> { wishlistCheck?.some((i) => i === product.id) ? removeProduct(product.id) : addProductToWishList(product.id)} } ><i className={`fa-solid fa-heart ${wishlistCheck.some((i) => i == product.id) ? "text-red-500 " : "text-[#777]"}`} ></i></button>
          <Link to={`/ProductDetails/${product.id}`}><i className="fa-solid fa-eye mt-3 text-[#777]"></i></Link>
        </div>
